...
try{
 ...
 browser=await chromium.launch({channel:'chrome',headless:true});const oc=await browser.newContext({viewport:{width:412,height:915}}),tc=await browser.newContext({viewport:{width:412,height:915}}),op=await oc.newPage(),tp=await tc.newPage();[op,tp].forEach(p=>p.setDefaultTimeout(30000));await login(op,owner);await login(tp,tech);await button(op,'Log hours for QA queue A').waitFor();await button(tp,'Log hours for QA queue A').waitFor();
