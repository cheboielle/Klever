...
try{
 ...
 phase='read-only access retains queued reading';await button(page,'Back to tasks').click();await page.getByRole('tab',{name:/Assets/}).click();await page.getByRole('button').filter({has:page.getByText('QA evidence asset',{exact:true})}).click();await context.setOffline(true);await field(page,'Current meter reading (hours)').fill('105');await button(page,'Save reading').click();await page.getByText('Saved on this device · 1 awaiting sync',{exact:true}).waitFor();
 ...
 phase='read-only export and restored access sync';await page.getByRole('button').filter({has:page.getByText('QA evidence asset',{exact:true})}).click();await button(page,'Manage asset').click();await button(page,'Details & photo').click();assert.equal(await button(page,'Edit asset details').isDisabled(),true);await button(page,'Exports & archive').click();await page.getByRole('button',{name:/Export this asset/}).click();const download=page.waitForEvent('download');await button(page,'CSV').first().click();mkdirSync('tmp/browser-evidence',{recursive:true});await (await download).saveAs('tmp/browser-evidence/readonly-assets.csv');assert.ok(readFileSync('tmp/browser-evidence/readonly-assets.csv','utf8').includes('QA evidence asset'));await page.getByText('Close',{exact:true}).click();
