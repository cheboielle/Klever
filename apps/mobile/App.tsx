import {ThemeContext,themes,type ThemeName} from './src/brandUI';
...
function Workspace(){
 const [brandTheme,setBrandTheme]=useState<ThemeName>('forest'),[logoVersion,setLogoVersion]=useState(0);
  ...
  const clearData=useCallback(()=>{
    ...
    setBrandTheme('forest');joiningRef.current=false;setJoining(false);setInvitationMode(false);
  ...
  const refresh=useCallback(async()=>{
    ...
    try{
      ...
      if(ticket!==generation.current)return;setBrandTheme(branding.theme);setLogoVersion(logo.revision);
      ...
      if(ticket!==generation.current)return;
      await saveCache('workspace',{theme:branding.theme,access:a,assets:results[0].data,types:results[1].data,members:results[2].data,ownerId:results[3].data?.owner_user_id??null});
      ...
      const cached=networkFailure(e)?readCache<{theme?:ThemeName;access:Access;assets:Asset[];members:Member[];types:AssetType[];ownerId?:string|null}>('workspace'):undefined;
      if(cached){setBrandTheme(cached.theme??'forest');setAccess(currentOffline()?.snapshot().access??cached.access);setAssets(cached.assets);setMembers(cached.members);setTypes(cached.types);setOwnerId(cached.ownerId??null);setError('Offline — showing saved data. New entries stay on this device until they sync.');
  ...
  return <ThemeContext.Provider value={brandTheme}><SafeAreaView style={[s.root,{backgroundColor:themes[brandTheme].paper}]}><StatusBar style="dark"/>
    <View style={s.top}><View style={[s.brand,{flex:1,minWidth:0}]}><ProfilePhoto key={access.tenant_id+':'+logoVersion} kind="business" target={access.tenant_id!} header/><View style={{flex:1,minWidth:0}}><Text style={s.brandText}>KLEVER ASSETS</Text><Text style={s.small} numberOfLines={1} ellipsizeMode="tail">{access.tenant_name}</Text></View></View><Pressable accessibilityRole="button" accessibilityLabel="Open menu" accessibilityState={{expanded:menuOpen}} onPress={()=>setMenuOpen(true)} style={{padding:12,flexShrink:0}}><Ionicons name="menu-outline" size={30} color={colors.ink}/></Pressable></View>
    ...
    <Modal visible={Boolean(selected)} animationType="slide" onRequestClose={()=>setSelected(null)}><View style={s.root}><FormScroll contentContainerStyle={s.modal} keyboardShouldPersistTaps="handled"><View style={s.sectionRow}><Text style={s.eyebrow}>{quickReading?"RECORD READING":"ASSET DETAILS"}</Text><Pressable onPress={()=>setSelected(null)}><Text style={s.link}>Close</Text></Pressable></View>
      ...
      {!quickReading?<><>{selected?<View style={{gap:24}}><DetailTile icon="construct-outline" title="Services" description="Due maintenance and service records"><ServicePanel key={selected.id+scheduleVersion} asset={selected} admin={Boolean(admin)} writable={assetWritable} captureWritable={captureWritable&&!selected?.archived}/></DetailTile><DetailTile icon="checkbox-outline" title="Tasks" description="Checklists and recurring work"><TaskPanel key={selected.id+scheduleVersion} asset={selected} assets={activeAssets} admin={Boolean(admin)} writable={assetWritable} captureWritable={captureWritable&&!selected?.archived}/></DetailTile></View>:null}</>
      <DetailTile key={(selected?.id??"")+":care"} icon="shield-checkmark-outline" title="Issues & compliance" description="Report problems and view important dates"><>{selected?<AssetCare key={selected.id} assetId={selected.id} admin={Boolean(admin)} writable={assetWritable} captureWritable={captureWritable&&!selected?.archived}/>:null}</></DetailTile>
