...
import {FeedbackHaptics} from './src/SaveFeedback';
import {AttentionPanel} from './src/AttentionPanel';
import {serviceSummary,taskSummary,type Overview} from './src/workspaceOverview';
import {ThemeContext,resolveTheme,type ThemeName} from './src/brandUI';
import type {Branding} from './src/BrandingSettings';
...
import {SafeAreaView} from './src/brandUI';
import {KeyboardProvider} from 'react-native-keyboard-controller';
import {Ionicons} from './src/BrandIcons';
import React,{useCallback,useEffect,useRef,useState} from 'react';
import {ActivityIndicator,AppState,KeyboardAvoidingView,Modal,Platform,Pressable,RefreshControl,ScrollView,StyleSheet,Text,TextInput,View} from './src/brandUI';
...
export default function App(){return <SafeAreaProvider><MotionProvider><KeyboardProvider><FeedbackHaptics/><Workspace/><InvitationLink/></KeyboardProvider></MotionProvider></SafeAreaProvider>;}
function Workspace(){
 const [overview,setOverview]=useState<Overview>();
 const [brandTheme,setBrandTheme]=useState<ThemeName>('forest'),[logoVersion,setLogoVersion]=useState(0);
  ...
  const clearData=useCallback(()=>{
    ...
    setOverview(undefined);setBrandTheme('forest');joiningRef.current=false;setJoining(false);setInvitationMode(false);
  ...
  const refresh=useCallback(async()=>{
    ...
    try{
      ...
      const [branding,logo]=await Promise.all([rpc<Branding>('business_branding'),rpc<{revision:number}>('get_profile_photo',{p_kind:'business',p_target:a.tenant_id})]);
      if(ticket!==generation.current)return;setBrandTheme(branding.theme);setLogoVersion(logo.revision);setLogoVersion(logo.revision);
      ...
      if(ticket!==generation.current)return;
      const summary=await rpc<Overview>('workspace_overview');if(ticket!==generation.current)return;setOverview(summary);
      ...
      if(ticket!==generation.current)return;
      await saveCache('workspace',{overview:summary,theme:branding.theme,access:a,assets:results[0].data,types:results[1].data,members:results[2].data,ownerId:results[3].data?.owner_user_id??null});
      ...
      const cached=networkFailure(e)?readCache<{overview?:Overview;theme?:ThemeName;access:Access;assets:Asset[];members:Member[];types:AssetType[];ownerId?:string|null}>('workspace'):undefined;
      if(cached){setOverview(cached.overview);setBrandTheme(cached.theme??'forest');setAccess(currentOffline()?.snapshot().access??cached.access);setAssets(cached.assets);setMembers(cached.members);setTypes(cached.types);setOwnerId(cached.ownerId??null);setError('Offline — showing saved data. New entries stay on this device until they sync.');
  ...
  return <ThemeContext.Provider value={brandTheme}><SafeAreaView style={[s.root,{backgroundColor:resolveTheme(brandTheme).paper}]}><StatusBar style="dark"/>
    <View style={[s.top,{borderBottomWidth:3,borderBottomColor:resolveTheme(brandTheme).brand}]}><View style={[s.brand,{flex:1,minWidth:0}]}><ProfilePhoto key={access.tenant_id+':'+logoVersion} kind="business" target={access.tenant_id!} header/><View style={{flex:1,minWidth:0}}><Text style={{fontSize:18,fontWeight:'800',color:colors.ink}} numberOfLines={2}>{access.tenant_name}</Text><Text style={s.small}>Equipment & maintenance</Text></View></View><Pressable accessibilityRole="button" accessibilityLabel="Open menu" accessibilityState={{expanded:menuOpen}} onPress={()=>setMenuOpen(true)} style={{padding:12,flexShrink:0}}><Ionicons name="menu-outline" size={30} color={colors.ink}/></Pressable></View>
    <FormScroll contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={loading} onRefresh={()=>void refresh()} tintColor={colors.green}/> }>
      ...
      {page==='assets'?<>
 <AttentionPanel overview={overview} assets={assets} online={online} onOpen={item=>{if(!online){const asset=assets.find(a=>a.id===item.assetId);if(item.kind==='task_due'){setTaskAlert({id:item.recordId!,asset:asset??null});setSelected(null);setPage('tasks');}else if(asset)void openAsset(asset,item.kind==='hour_log',item.kind==='service_due'?item.recordId!:null);return;}void openAlert(item).then(ok=>{if(!ok)setError('Unable to open this item. Reconnect and try again.');}).catch(()=>setError('Unable to open this item. Reconnect and try again.'));}}/>
        ...
        {filtered.length?filtered.map(a=><Pressable key={a.id} accessibilityRole="button" onPress={()=>void openAsset(a)} style={s.assetCard}>
          <ProfilePhoto key={a.id+':'+photoVersion} kind="asset" target={a.id} compact/><View style={{flex:1,gap:5}}><Text style={s.assetName}>{a.name}</Text><Text style={s.small}>{a.serial||'No serial recorded'}</Text><Text style={{fontSize:12,color:colors.amber}}>{a.archived?'Historic records':overview?.assets.find(row=>row.id===a.id)?.open_issues?`${overview.assets.find(row=>row.id===a.id)!.open_issues} open issue(s)`:serviceSummary(overview?.assets.find(row=>row.id===a.id))}</Text><Text style={[s.badge,{color:a.status==='Active'?colors.green:colors.amber}]}>{a.archived?'Archived':a.status}</Text></View><View style={{alignItems:'flex-end',gap:5}}><Text style={s.hours}>{Number(a.current_hours).toLocaleString()}</Text><Text style={s.small}>{a.meter_unit}</Text><Text style={s.link}>View details →</Text>{!a.archived?<Pressable accessibilityRole="button" accessibilityLabel={`Log ${a.meter_unit} for ${a.name}`} disabled={!captureWritable} onPress={event=>{event.stopPropagation();void openAsset(a,true);}} style={{padding:10,minHeight:44,borderRadius:10,backgroundColor:"#EAF0E7",opacity:captureWritable?1:.4}}><Text style={s.link}>Log {a.meter_unit}</Text></Pressable>:null}</View>
    ...
    <Modal visible={Boolean(selected)} animationType="slide" onRequestClose={()=>{setSelected(null);void refresh();}}><View style={s.root}><FormScroll contentContainerStyle={s.modal} keyboardShouldPersistTaps="handled"><View style={s.sectionRow}><Text style={s.eyebrow}>{quickReading?"RECORD READING":"ASSET DETAILS"}</Text><Pressable onPress={()=>{setSelected(null);void refresh();}}><Text style={s.link}>Close</Text></Pressable></View>
      ...
      <DetailTile key={(selected?.id??"")+":reading"} icon="speedometer-outline" title="Log a reading" description="Record the current hours or kilometres" initiallyOpen><Field label={selected?.meter_unit==='km'?'Current odometer reading (km)':'Current meter reading (hours)'} value={hours} numeric onChangeText={v=>{pendingReading.current=null;setHours(v);}}/><Button title={busy?'Saving…':'Save reading'} onPress={()=>void saveReading()} disabled={busy||(!captureWritable||Boolean(selected?.archived))||!hours.trim()}/></DetailTile>
      {error?<Notice text={error}/>:null}
      {!quickReading?<><>{selected?<View style={{gap:24}}><DetailTile onCollapse={()=>void refresh()} icon="construct-outline" title="Services" description={selected?.archived?"Archived asset · Service records":serviceSummary(overview?.assets.find(a=>a.id===selected?.id))}><ServicePanel key={selected.id+scheduleVersion} asset={selected} admin={Boolean(admin)} writable={assetWritable} captureWritable={captureWritable&&!selected?.archived}/></DetailTile><DetailTile onCollapse={()=>void refresh()} icon="checkbox-outline" title="Tasks" description={selected?.archived?"Archived asset · Task records":taskSummary(overview?.assets.find(a=>a.id===selected?.id))}><TaskPanel key={selected.id+scheduleVersion} asset={selected} assets={activeAssets} admin={Boolean(admin)} writable={assetWritable} captureWritable={captureWritable&&!selected?.archived}/></DetailTile></View>:null}</>
      <DetailTile onCollapse={()=>void refresh()} key={(selected?.id??"")+":care"} icon="shield-checkmark-outline" title="Issues & compliance" description={overview?`${overview.assets.find(a=>a.id===selected?.id)?.open_issues??0} open issues · Compliance dates`:"Loading current issues…"}><>{selected?<AssetCare key={selected.id} assetId={selected.id} admin={Boolean(admin)} writable={assetWritable} captureWritable={captureWritable&&!selected?.archived}/>:null}</></DetailTile>
      {admin?<DetailTile key={(selected?.id??"")+":manage"} icon="settings-outline" title="Manage asset" description="Details, assignments, corrections and setup">
      <DetailTile icon="image-outline" title="Details & photo">      {admin?<Button title="Edit asset details" secondary disabled={!assetWritable||busy} onPress={()=>{setEditingAsset(selected);setNewName(selected!.name);setSerial(selected!.serial);setTypeId(selected!.asset_type_id);setInitialHours(String(selected!.current_hours));setMeterUnit(selected!.meter_unit);setMeterReason('');setMeterConfirmed(false);setError('');setSelected(null);setShowAdd(true);}}/>:null}{selected?<ProfilePhoto key={selected.id} kind="asset" target={selected.id} editable writable={assetWritable} onChanged={()=>setPhotoVersion(v=>v+1)}/>:null}</DetailTile>
      <DetailTile icon="pulse-outline" title="Asset status">      <SelectField label="Choose status" value={nextStatus} options={statuses.map(value=>({value,label:value}))} onChange={setNextStatus}/><Field label="Reason for change" value={reason} onChangeText={setReason}/><Button title="Update status" secondary disabled={!assetWritable||busy||!reason.trim()} onPress={()=>void run(async()=>{await rpc('set_asset_status',{p_asset:selected!.id,p_status:nextStatus,p_reason:reason.trim()});setSelected(null);await refresh();})}/></DetailTile>
      <DetailTile icon="people-outline" title="Assigned staff">        {members.filter(m=>m.is_active).map(m=><Pressable key={m.user_id} disabled={!assetWritable||busy} style={s.sectionRow} onPress={()=>void run(async()=>{await rpc('assign_asset',{p_asset:selected!.id,p_user:m.user_id,p_assigned:!assignmentIds.includes(m.user_id)});setAssignmentIds(ids=>ids.includes(m.user_id)?ids.filter(x=>x!==m.user_id):[...ids,m.user_id]);})}><Text style={s.label}>{m.name}</Text><Text style={s.link}>{assignmentIds.includes(m.user_id)?'Assigned ✓':'Assign +'}</Text></Pressable>)}</DetailTile>
      <DetailTile icon="create-outline" title="Correct a reading">{admin&&selected?<ReadingCorrection key={selected.id+':'+selected.meter_revision} asset={selected} writable={assetWritable} onSaved={async()=>{setSelected(null);await refresh();}}/>:null}</DetailTile>
      <DetailTile icon="albums-outline" title="Starter service templates"><>{admin&&selected?<StarterLibrary asset={selected} writable={assetWritable} onApplied={()=>setScheduleVersion(v=>v+1)}/>:null}</></DetailTile>
      <DetailTile icon="archive-outline" title="Exports & archive">      {admin&&selected?<ExportPanel assetId={selected.id}/>:null}{admin&&selected?<AssetArchive asset={selected} writable={writable} onSaved={async()=>{setSelected(null);await refresh();}}/>:null}</DetailTile>
      </DetailTile>:null}
      <DetailTile key={(selected?.id??"")+":history"} icon="time-outline" title="Reading history" description="Previous readings and recorded corrections">      {meterChanges.length?<><Text style={s.heading}>Meter unit corrections</Text>{meterChanges.map(change=><View style={s.card} key={change.id}><Text style={s.label}>{change.details.previous_reading} {change.details.previous_unit} → {change.details.reading} {change.details.meter_unit}</Text><Text style={s.muted}>{change.reason}</Text><Text style={s.small}>{new Date(change.server_time).toLocaleString()}</Text></View>)}</>:null}
