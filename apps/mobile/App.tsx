...
type HourLog={meter_unit:MeterUnit;id:string;value:string|number;delta:string|number;capture_time:string;server_time:string;reason:string|null};
...
export default function App(){
  ...
  const refresh=useCallback(async(unlock=true)=>{
    ...
    try{
      ...
      if(cached){setAccess(currentOffline()?.snapshot().access??cached.access);setAssets(cached.assets);setMembers(cached.members);setTypes(cached.types);setError('Offline â€” showing saved data. New entries stay on this device until they sync.');
  ...
  async function openAsset(asset:Asset){
    ...
    await run(async()=>{
      const [history,assigned,changes]=await Promise.all([
        supabase!.from('hour_logs').select('id,value,delta,capture_time,server_time,reason,meter_unit').eq('asset_id',asset.id).order('revision',{ascending:false}).limit(20),
  ...
  if(boot)return <View style={s.center}><ActivityIndicator color={colors.green}/><Text style={s.muted}>Opening your workspaceâ€¦</Text></View>;
  if(!configured)return <View style={s.center}><Text style={s.title}>Connection needed</Text><Text style={s.muted}>The appâ€™s Supabase connection must be configured before signing in.</Text></View>;
  if(!session)return <KeyboardAvoidingView style={s.root} behavior={Platform.OS==='ios'?'padding':undefined}><StatusBar style="dark"/>
    <ScrollView contentContainerStyle={s.login} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.card}><Text style={s.heading}>Welcome back</Text><Text style={s.muted}>Sign in to your business workspace.</Text>
        ...
        {error?<Notice text={error}/>:null}<Button title={busy?'Signing inâ€¦':'Sign in'} onPress={()=>void login()} disabled={busy||!email.trim()||!password}/>
  ...
  return <View style={s.root}><StatusBar style="dark"/>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={loading} onRefresh={()=>void refresh()} tintColor={colors.green}/> }>
      ...
      <Text style={s.subtitle}>Good to see you, {access.name?.split(' ')[0]}.</Text><NotificationSetup/>{admin?<><NotificationSettings writable={writable}/><ExportPanel/></>:null}
    ...
    <Modal visible={showAdd} animationType="slide" onRequestClose={()=>setShowAdd(false)}><View style={s.root}><ScrollView contentContainerStyle={s.modal} keyboardShouldPersistTaps="handled"><View style={s.sectionRow}><Text style={s.title}>{editingAsset?'Edit asset':'Add an asset'}</Text><Pressable onPress={()=>setShowAdd(false)}><Text style={s.link}>Close</Text></Pressable></View>
      ...
      {editingAsset&&meterUnit!==editingAsset.meter_unit?<><Notice text="Enter the correct current reading in the new unit. Earlier readings keep their original units."/><Field label="Reason for unit correction" value={meterReason} onChangeText={setMeterReason}/><Pressable accessibilityRole="checkbox" accessibilityState={{checked:meterConfirmed}} onPress={()=>setMeterConfirmed(!meterConfirmed)} style={s.chip}><Text style={s.label}>{meterConfirmed?'âœ“ ':''}I confirm {initialHours||'the reading'} {meterUnit} is correct</Text></Pressable></>:null}
    ...
    <Modal visible={Boolean(selected)} animationType="slide" onRequestClose={()=>setSelected(null)}><View style={s.root}><ScrollView contentContainerStyle={s.modal} keyboardShouldPersistTaps="handled"><View style={s.sectionRow}><Text style={s.eyebrow}>ASSET DETAILS</Text><Pressable onPress={()=>setSelected(null)}><Text style={s.link}>Close</Text></Pressable></View>
      ...
      {admin&&selected?<ExportPanel assetId={selected.id}/>:null}<>{selected?<AssetCare key={selected.id} assetId={selected.id} admin={Boolean(admin)} writable={writable} captureWritable={captureWritable}/>:null}</><Text style={s.heading}>Log a reading</Text><Field label={selected?.meter_unit==='km'?'Current odometer reading (km)':'Current meter reading (hours)'} value={hours} numeric onChangeText={v=>{pendingReading.current=null;setHours(v);}}/><Button title={busy?'Savingâ€¦':'Save reading'} onPress={()=>void saveReading()} disabled={busy||!captureWritable||!hours.trim()}/>
