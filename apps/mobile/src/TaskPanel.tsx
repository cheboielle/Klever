...
export function TaskPanel({asset=null,assets,admin,writable,captureWritable=writable,initialTaskId}:{asset?:Asset|null;assets:Asset[];admin:boolean;writable:boolean;captureWritable?:boolean;initialTaskId?:string}){
 ...
 const [today,setToday]=useState('');const generation=useRef(0),handledAlert=useRef<string|undefined>(undefined);
 useEffect(()=>{if(!initialTaskId||loading||error||handledAlert.current===initialTaskId)return;handledAlert.current=initialTaskId;const target=items.find(item=>item.id===initialTaskId&&!item.archived);if(!target)setError('This task is no longer available to you.');else if(!target.due)setError('This task is already completed or is not due yet. The current schedule is shown below.');else setOpen(target);},[initialTaskId,loading,error,items]);
 ...
 return <View style={s.group}><Text style={s.heading}>{asset?'Asset tasks':'Business tasks'}</Text>{!asset?<Text style={s.text}>Jobs for the whole team, completed once for everyone or separately by each technician.</Text>:null}
  ...
  {editing===undefined?<>
   ...
   {cfg.cadence==='custom'?<IntervalField value={days} onChange={setDays} disabled={busy||!writable}/>:null}<DateField label="Next due date" value={date} onChange={setDate} disabled={busy||!writable}/>
