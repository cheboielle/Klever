...
export function NotificationSetup({onOpenAlert,showControls=true}:{showControls?:boolean;onOpenAlert:(data:unknown)=>Promise<boolean>}){
 ...
 async function openResponse(value:Notifications.NotificationResponse){
  try{const opened=await openAlert.current(value.notification.request.content.data);
   ...
   else {setRetry(value);setMessage('Sign in or reconnect, then open the alert again.');}
 ...
 return <View style={{gap:8}}>{showControls?<Pressable accessibilityRole="button" disabled={busy} onPress={()=>void enable()} style={{padding:12,borderRadius:10,backgroundColor:'#EAF0E7'}}><Text style={{fontWeight:'600',color:'#153C32'}}>{busy?'Registering phone…':'Enable phone alerts'}</Text></Pressable>:null}{retry?<Pressable accessibilityRole="button" onPress={()=>void openResponse(retry)} style={{padding:12,backgroundColor:'#EAF0E7'}}><Text>Open alert again</Text></Pressable>:null}{message&&(showControls||retry)?<Text style={{fontSize:13,color:'#53665B'}}>{message}</Text>:null}</View>;
}
