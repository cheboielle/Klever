...
export function ServiceHistory({asset,service,admin,writable,onCorrected,allowCorrection=true}:{asset:Asset;service:{id:string;config:ServiceConfig;settings_revision:number;baseline_reading:number|null;baseline_date:string|null};admin:boolean;writable:boolean;onCorrected:()=>void;allowCorrection?:boolean}){
  ...
  return <View style={{gap:12}}><Text style={s.label}>Service history</Text>{error?<Text style={s.error}>{error}</Text>:null}{records.length?records.map(record=><View key={record.id} style={s.card}>
    ...
    {correcting===record.id?<View style={s.card}><Text style={s.label}>Correct this service record</Text><Button label={action==='replace'?'✓ Correct record':'Correct record'} onPress={()=>setAction('replace')}/><Button label={action==='void'?'✓ Void record':'Void record'} onPress={()=>setAction('void')}/><Text style={s.text}>The original photo and entry stay visible. Confirm the baseline the schedule should use after this correction; leave unknown values blank.</Text><TextInput accessibilityLabel="Correction reason" placeholder="Reason for correction" value={reason} onChangeText={setReason} style={s.input}/>{service.config.mode!=='calendar'?<><Text style={s.label}>Baseline reading ({asset.meter_unit})</Text><TextInput accessibilityLabel="Corrected baseline reading" value={baseline} onChangeText={v=>{setBaseline(v);setConfirmed(false);}} keyboardType="decimal-pad" style={s.input}/></>:null}{service.config.mode!=='meter'?<><DateField label="Corrected baseline date" value={date} onChange={v=>{setDate(v);setConfirmed(false);}} clearable/></>:null}<Pressable accessibilityRole="checkbox" accessibilityState={{checked:confirmed}} onPress={()=>setConfirmed(!confirmed)}><Text style={s.label}>{confirmed?'✓ ':''}I confirm this correction and baseline</Text></Pressable><Button label="Save correction" onPress={()=>void correct()} disabled={busy||!confirmed||!writable}/><Button label="Cancel correction" onPress={()=>setCorrecting(null)} disabled={busy}/></View>:null}
