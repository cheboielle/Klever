...
type History={corrections:{action:string;reason:string;baseline_reading:number|null;baseline_date:string|null;meter_unit:string;server_time:string}[];id:string;state:string;capture_time:string;server_time:string;reading:number;meter_unit:string;performer_name:string;snapshot:ServiceConfig;object_path:string;cost:number|null;mechanic_notes:string|null};
...
export function ServiceCompletion({asset,service,admin,writable,onSaved,onCancel}:{asset:Asset;service:{id:string;config:ServiceConfig;settings_revision:number};admin:boolean;writable:boolean;onSaved:()=>void;onCancel:()=>void}){
  ...
  return <View style={s.card}>
    ...
    {admin?<><Text style={s.label}>Cost (optional)</Text><TextInput accessibilityLabel="Service cost" value={cost} onChangeText={setCost} keyboardType="decimal-pad" style={s.input} editable={!busy}/><Text style={s.label}>Mechanic notes (admin only)</Text><TextInput accessibilityLabel="Mechanic notes" value={notes} onChangeText={setNotes} multiline style={s.input} editable={!busy}/></>:null}
...
export function ServiceHistory({asset,service,admin,writable,onCorrected}:{asset:Asset;service:{id:string;config:ServiceConfig;settings_revision:number;baseline_reading:number|null;baseline_date:string|null};admin:boolean;writable:boolean;onCorrected:()=>void}){
  ...
  return <View style={{gap:12}}><Text style={s.label}>Service history</Text>{error?<Text style={s.error}>{error}</Text>:null}{records.length?records.map(record=><View key={record.id} style={s.card}>
    ...
    <Text style={s.text}>{record.snapshot.instructions}</Text>{record.cost!==null?<Text style={s.text}>Cost: {Number(record.cost).toFixed(2)}</Text>:null}{record.mechanic_notes?<Text style={s.text}>{record.mechanic_notes}</Text>:null}
    ...
    {admin&&correcting!==record.id?<Button label="Correct or void this record" onPress={()=>editCorrection(record)} disabled={busy||!writable}/>:null}
