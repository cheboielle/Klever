import {useBrandTheme} from './brandUI';
import React from 'react';
import {Text,TextInput,View} from './brandUI';
...
export function IntervalField({value,onChange,disabled=false}:{value:string;onChange:(value:string)=>void;disabled?:boolean}){
 ...
 return <View style={{gap:10}}>
  ...
  <Slider accessibilityLabel="Quick interval, 1 to 30 days" minimumValue={1} maximumValue={30} step={1} value={Number.isFinite(numeric)?Math.min(30,Math.max(1,numeric)):1} disabled={disabled} onValueChange={n=>onChange(String(Math.round(n)))} minimumTrackTintColor={theme.accent} maximumTrackTintColor="#DCE5DC" thumbTintColor={theme.accent} style={{height:48,width:'100%'}}/>
