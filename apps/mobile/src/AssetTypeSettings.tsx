...
export function AssetTypeSettings({types,writable,onSaved}:{types:{id:string;name:string}[];writable:boolean;onSaved:()=>Promise<void>}){
 ...
 return <>{button('Asset types',()=>setOpen(true))}<Modal visible={open} onRequestClose={()=>setOpen(false)} animationType="slide"><ScrollView contentContainerStyle={{padding:24,paddingTop:50,gap:16,backgroundColor:'#F5F6F0'}}>
 ...
 {message?<Text accessibilityLiveRegion="polite">{message}</Text>:null}</ScrollView></Modal></>;
}
