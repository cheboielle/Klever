...
export function BusinessSettings({writable,onSaved}:{writable:boolean;onSaved:()=>Promise<void>}){
 ...
 <Modal visible={open} animationType="slide" onRequestClose={()=>setOpen(false)}><ScrollView contentContainerStyle={{padding:24,paddingTop:50,gap:15,backgroundColor:'#F5F6F0'}} keyboardShouldPersistTaps="handled">
 ...
 <Text>Currency for new service costs</Text>{['NZD','AUD','USD','CAD','GBP','EUR'].map(currency=><Button key={currency} label={`${settings.currency===currency?'✓ ':''}${currency}`} disabled={!writable||busy} onPress={()=>change({currency})}/>)}
