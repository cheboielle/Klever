...
Windows dependency recovery: the verified workspace node_modules layout is isolated. Use `pnpm --config.node-linker=isolated install --frozen-lockfile`; do not force a hoisted relink over this tree based on the historical .npmrc. A forced hoisted repair produced temporary import ENOENT errors; restoring the original isolated layout passed all checks. Expo Doctor needs npm on PATH even though pnpm manages this app; this host can run it with `pnpm --config.node-linker=hoisted --package=npm@latest --package=expo-doctor@latest dlx expo-doctor` from apps/mobile. That temporary tool environment does not change the app package manager. Native icon fonts explicitly depend on SDK-compatible expo-font 57.0.3, per [Expo Font documentation](https://docs.expo.dev/versions/latest/sdk/font/).
