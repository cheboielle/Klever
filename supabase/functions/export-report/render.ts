...
const columns:Record<string,string[]>={assets:['id','name','serial','type','status','meter_unit','current_reading','archived'],logs:['id','asset','reading','unit','delta','performer','capture_time','server_time','correction_of','reason'],services:['id','asset','serial','service','reading','unit','performer','capture_time','server_time','state','cost','mechanic_notes','instructions','corrections','photo_path'],tasks:['id','asset','task','performer','capture_time','server_time','due_date','effective_completion','notes','instructions','checked_ids','corrections','photo_path']};
...
export async function renderPdf(data:ExportData,fontBytes:Uint8Array,photo:(row:Record<string,any>)=>Promise<Uint8Array|null>){
 ...
 for(let index=0;index<data.rows.length;index++){
  ...
  if(data.kind==='services'||data.kind==='tasks'){
   ...
   if(row.notes)text('Notes: '+row.notes);if(row.cost!=null)text('Recorded cost: '+row.cost);if(row.mechanic_notes)text('Mechanic notes: '+row.mechanic_notes);
