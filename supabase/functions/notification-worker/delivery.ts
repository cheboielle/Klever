...
export async function deliver(job:DeliveryJob,config:Config,send:typeof fetch=fetch,deadline=Date.now()+80000):Promise<Outcome>{
 const timeout=()=>AbortSignal.timeout(Math.max(1,Math.min(20000,deadline-Date.now())));const state={...job.delivery_state};const subject=job.event_type==='urgent_issue'?`Urgent issue: ${job.payload.asset_name??'asset'}`:'Klever Assets reminder';const body=String(job.payload.description??job.payload.message??'Open Klever Assets to see the latest maintenance information.');
 try{
  if(Date.now()>=deadline)return {status:'pending',state,error:'Continuing delivery on the next attempt'};
  if(job.channel==='email'){
   ...
   const response=await send('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${config.resendKey}`,'Content-Type':'application/json','Idempotency-Key':job.id},body:JSON.stringify({from:config.from,to:[job.email],subject,text:body}),signal:timeout()});
  ...
  for(const device of job.devices){
   ...
   if(prior?.ticket){
    const response=await send('https://exp.host/--/api/v2/push/getReceipts',{method:'POST',headers,body:JSON.stringify({ids:[prior.ticket]}),signal:timeout()});const data=await response.json();
