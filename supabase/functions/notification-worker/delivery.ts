...
export async function deliver(job:DeliveryJob,config:Config,send:typeof fetch=fetch):Promise<Outcome>{
 ...
 try{
  ...
  for(const device of job.devices){
   ...
   const response=await send('https://exp.host/--/api/v2/push/send',{method:'POST',headers,body:JSON.stringify({to:device.token,title:subject.slice(0,180),body:body.slice(0,240),sound:'default',data:{assetId:job.payload.asset_id??null,kind:job.event_type}}),signal:AbortSignal.timeout(20000)});const data=await response.json();
