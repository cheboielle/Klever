...
Owner-approved UX refinement (12 September 2026): the administrator can enter an exact HEX brand colour or choose it using a colour wheel, saturation/brightness sliders and quick swatches. Preview the shared business theme before saving, retaining the exact chosen colour while deriving readable text and softer backgrounds. Lead the header with the business name and optional existing logo; retain Klever identity in About. Show current service/task/issue summaries on grouped asset tiles, clearer asset cards and direct reading actions. A Needs attention section combines existing due services, tasks and configured reading reminders, opening their focused record screens through current permission checks. Add restrained save/sync confirmation, native haptic feedback, pressed states and reduced-motion-aware tile expansion; use consistent sheets for short choices. These refinements preserve existing functionality and permission/offline guarantees; they do not introduce new reminder rules or task types.
...
Owner branding addition (12 September 2026): the admin selects one shared business colour theme and can upload, replace or remove a business logo in Business settings. Apply the theme and logo to that business's signed-in experience for all members. Display the logo in place of the top-left K box, fitting the whole image without distortion; retain the K fallback when no logo is available. Keep the selected theme readable, including controls and feedback. This customizes the in-app workspace, not the installed app icon or store listing. Persist branding per tenant with current-access reads and admin/write-entitlement enforcement on changes. Reuse private media storage and recoverable photo handling. No separate per-person theme setting is required.
