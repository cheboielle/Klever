...
12 September owner-approved UX refinement: extend the existing tenant branding command to validated HEX values with revision checks, reuse private logo delivery, and derive readable presentation colours locally. Read the due-work overview through authenticated, assignment-filtered database functions using the existing service/task/reminder calculations. Keep the existing secure notification destinations and durable entry queue. Save feedback must distinguish local persistence from server acceptance and must never affect submission durability; native haptics are optional presentation feedback. Refresh collapsed summaries after administration and keep expanded form drafts mounted. Validate custom colours, direct record destinations, tenant/role access, offline feedback and existing photo/calendar journeys, then produce one combined internal Android review build. Physical-phone colour gestures, sheet/keyboard behaviour and haptics remain device acceptance checks.
...
12 September branding implementation: add tenant-scoped theme and logo settings with an admin editor in Business settings. Use shared theme tokens throughout the signed-in screens and existing private upload/download patterns for logos; do not change service evidence. Preview the selected theme/logo before saving, preserve the current branding after a failed upload, and offer removal back to the K fallback. Verify tenant isolation, technician read/admin write permissions, revoked/read-only access, logo replacement/removal, and readable phone layouts. Package with the requested navigation icons and staff-photo creation-flow improvement in the next Android review build.
